"""
Unit tests for modules/admin/__init__.py
"""
import pytest
from fastapi import APIRouter
from modules.admin import router, get_module_info
...
class TestAdminModule:
    ...
    def test_router_exists(self):
        """Test that admin router is exported"""
    ...
    def test_get_module_info_name(self):
        """Test that module name is 'admin'"""
        info = get_module_info()
        assert info["name"] == "admin"
    
    def test_get_module_info_routes(self):
        """Test that module declares admin routes"""
        info = get_module_info()
        
        assert isinstance(info["routes"], list)
        assert "/admin/*" in info["routes"]
    ...
    def test_router_prefix_matches_routes(self):
        ...
        assert router.prefix == "/admin"
        assert any("/admin" in route for route in info["routes"])