"""
Unit tests for modules/forums/__init__.py
"""
import pytest
from fastapi import APIRouter
from modules.forums import router, get_module_info


class TestForumsModule:
    """Test forums module initialization"""
    
    def test_router_exists(self):
        """Test that forums router is exported"""
    ...
    def test_get_module_info_name(self):
        """Test that module name is 'forums'"""
        info = get_module_info()
        assert info["name"] == "forums"
    
    def test_get_module_info_routes(self):
        """Test that module declares forums routes"""
        info = get_module_info()
        
        assert isinstance(info["routes"], list)
        assert "/forums/*" in info["routes"]
    ...
    def test_router_prefix_matches_routes(self):
        ...
        assert router.prefix == "/forums"
        assert any("/forums" in route for route in info["routes"])